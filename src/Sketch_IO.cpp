...
void saveKssdSketches_MPI(const string folderPath, const KssdParameters& info, const vector<KssdSketchInfo>& sketches, int my_rank, int comm_sz)
{
    string info_file_path = folderPath + "/kssd.info.sketch";
    ofstream info_ofs;
    if (my_rank == 0) {
        info_ofs.open(info_file_path, ios::out | ios::trunc); 
    } else {
        info_ofs.open(info_file_path, ios::out | ios::app);
    }

    if (!info_ofs.is_open()) {
        cerr << "ERROR [Rank " << my_rank << "]: Cannot open file: " << info_file_path << endl;
        MPI_Abort(MPI_COMM_WORLD, 1);
    }
    
    for (const auto& sketch_info : sketches) {
        info_ofs << sketch_info.fileName << "\t" 
                 << sketch_info.fileSeqs.size() << "\t" 
                 << sketch_info.totalSeqLength << "\t" 
                 << sketch_info.sketchsize << "\t"
                 << info.drlevel << "\n";
    }
    info_ofs.close();

    string hash_file_path = folderPath + "/kssd.hash.sketch";
    FILE* fp_hash = nullptr;

    if (my_rank == 0) {
        fp_hash = fopen(hash_file_path.c_str(), "wb");
        if (!fp_hash) {
            cerr << "ERROR [Rank 0]: Cannot open file for writing: " << hash_file_path << endl;
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
        fwrite(&info, sizeof(KssdParameters), 1, fp_hash);
    }
    else {
        fp_hash = fopen(hash_file_path.c_str(), "ab");
        if (!fp_hash) {
            cerr << "ERROR [Rank " << my_rank << "]: Cannot open file for appending: " << hash_file_path << endl;
            MPI_Abort(MPI_COMM_WORLD, 1);
        }
    }
